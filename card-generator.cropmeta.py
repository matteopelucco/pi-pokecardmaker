...
def _sync_crop_sidecar(rendered_json, sidecar_path: Path):
    """
    Keep crop parameters stable across generations.
    - If sidecar exists, load it and apply its crop params to all image objects.
    ...
    if sidecar_path.exists():
        try:
            params = json.loads(sidecar_path.read_text(encoding="utf-8"))
        except Exception:
            # If sidecar is corrupted, don't crash the whole generation; fall back to template.
            params = {}
        if isinstance(params, dict) and params:
            for img in image_dicts:
                _apply_crop_params_to_image(img, params)
        return

    # Create sidecar from template crop params (first image object that has any crop keys)
    params = {}
    for img in image_dicts:
        params = _extract_crop_params_from_image(img)
        if params:
            break

    if params:
        sidecar_path.write_text(json.dumps(params, ensure_ascii=False, indent=2), encoding="utf-8")
